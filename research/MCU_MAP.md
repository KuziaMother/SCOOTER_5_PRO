...
| [`0x16bd4`](functions_mcu/func_0x16bd4.md) | 442 | код G | **2D-билинейная интерполяция на u16-сетке**: values — stack-passed ([sp+0x1c]/[sp+0x20], артефакт кодогенератора); arg2=xtab1 (u16), arg3=ystruct {u32 x0, u32 count, u32 ys...}; [cs]=ygrid (u16, stride=[cs+8]=5), [cs+4]={u32 count xtab1}; part1: binsearch value1→idx1 + Q16-slope (прямой udiv); part2: binsearch value2→idx2 + Q16 (0x161ea n=0x10); yA=interp(grid[idx2*5+idx1],+1,slope1), yB=то же в строке idx2+1, result=yA±(yB−yA)*slope2>>16 (u16); caller: 0x69e4 (таблица flash @0x19F60: xtab1@+0xBC, ystruct@−0xAC, grid@+0x550) | §52 | разобран | 100% |
...
| [`0x1c34c`](functions_mcu/func_0x1c34c.md) | 1244 | код I | **Q20 position/velocity controller (мотор)**: byte[RAM+0x32A] ? u32[RAM+0x37C]=850 : 680 (0x55<<3); temp=s16[RAM+0x3C8+8]×240; v1=**0x19994**(temp, u32[RAM+0x3C8+0x4C]) (Q20 §52) → u32[RAM+0x3C8+0x50]; v2=**0x19994**(v1×65520, s16[RAM+0x1768+0x38]) → u32[RAM+0x224] = clamp(v2, 0, **0x7FF8 (32760)**); середина: r6/r5 = u32-пара [pool+0/+4] (позиция), byte mode (0/1/other): Q20 через **0x19968** с константами 0x64<<0xF, 0x2D<<4, пороги **0x1E (30)**/**0x4B (75)**, константа **0x7D00 (32000)**; запись u32[pool+0x80+0x18/+0x1C] (позиция/скорость x2), u16-flags: OR/CLEAR bit2, byte[+5]=0/1; signed-shift трюк (lsl #0x17 + bpl = знак); pool: RAM+0x32A/+0x3C8/+0x1768/+0x224/+0x33A/+0x30E/+0x311/+0x1E0/+0x233/+0x27C/+0x21E/+0x220/+0x231/+0x27E/+0x245/+0x230/+0x380; **callers: не найдены в bl-скане** (косвенный вызов?) | §54 | разобран | 100% |
...
| [`0x1d078`](functions_mcu/func_0x1d078.md) | 610 | код I | state-машина режимов (byte@0x229: 2/3/0x0B) — адрес приблизительный; **timer capture/period handler (код I)**: u32[RAM+0x154+4]+byte[RAM+0x100] → **0x1d874** (timer helper); strh r0, [RAM+0x1768]; u32[RAM+0x1768+0xC] + s16[+0x10]; bl 0x19994 (Q20-кластер §52); pool: RAM+0x154/+0x100/+0x1768/+0x236/+0x339/+0x326/+0x229/+0x333/+0x320/+0x3C8/+0x244/+0x1D0/+0x263/+0x224 | §34.2, §53 | разобран | 100% |
