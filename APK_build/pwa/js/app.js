...
const LTMK_KEY = "scooter_ltmk_hex";
...
function renderRideMode(state) {
  ...
  const chips = [];
  if (locked != null) {
    chips.push(`<button type="button" class="rm-chip rm-lock-btn ${locked ? "bad" : "ok"}" data-locked="${locked ? 1 : 0}">${locked ? "🔒 Заблокирован" : "🔓 Разблокирован"}</button>`);
  }
...
$("write-toggle").onclick = () => {
  const enabled = !appState.write_enabled;
  if (enabled && !confirm("Включить режим управления?\nКлик по функции будет МЕНЯТЬ настройку самоката (SET, необратимо до обратного клика).")) return;
...
function getLtmk() {
  let hex = localStorage.getItem(LTMK_KEY);
  if (!hex) {
    hex = (prompt(
      "Введите LTMK (64 hex-символа — 32 байта ключа логина самоката, см. secrets/ltmk.hex).\n" +
      "Хранится только в этом браузере (localStorage), никуда не отправляется."
    ) || "").trim();
    if (hex) localStorage.setItem(LTMK_KEY, hex);
  }
...
async function doSet(key, typeCode, value) {
